...
import { FaInstagram, FaLinkedin, FaGithub, FaMedium, FaXTwitter, FaYoutube, FaPinterest, FaReddit, FaHackerNews, FaThreads } from 'react-icons/fa6';
...
const socials = [
  ...
  { name: 'Threads',   href: 'https://www.threads.com/@aevrin.official',           icon: FaThreads },
];
...
export function SocialIcons() {
  ...
  return (
    <div className="relative flex items-center justify-between px-1 py-1 rounded-2xl w-full bg-neutral-100 dark:bg-neutral-950 border border-black/[0.08] dark:border-white/[0.08]">
      ...
      {socials.map(({ name, href, icon: Icon }, index) => {
        ...
        return (
          <a
            ...
            className="group relative flex items-center justify-center size-7 sm:size-9 rounded-lg sm:rounded-xl transition-colors duration-200 flex-shrink-0"
          >
            <span className={`absolute inset-1 rounded-md sm:rounded-lg bg-black/[0.06] dark:bg-white/[0.08] transition-all duration-300 ease-out ${hovered ? 'opacity-100 scale-100' : 'opacity-0 scale-90'}`} />

            <Icon
              style={{ display: 'block' }}
              className={`relative z-10 w-3 h-3 sm:w-[17px] sm:h-[17px] transition-all duration-300 ease-out ${hovered ? 'text-neutral-900 dark:text-white scale-110' : 'text-neutral-400 dark:text-neutral-500'}`}
            />

            <span className={`absolute bottom-1 sm:bottom-1.5 left-1/2 -translate-x-1/2 h-[2px] rounded-full bg-neutral-900 dark:bg-white transition-all duration-300 ease-out ${hovered ? 'w-2 sm:w-3 opacity-100' : 'w-0 opacity-0'}`} />
