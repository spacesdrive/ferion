...
export function BentoCard({
  ...
  return (
    <motion.div
      ...
      {/* Spotlight Effect */}
      {spotlight && (
        <Spotlight mouseX={spotlightX} mouseY={spotlightY} isHovered={isHovered} />
      )}
      {/* Border Beam */}
      ...
      {background && (
        <div
          className="absolute inset-0 z-0 opacity-50 transition-opacity duration-300 group-hover:opacity-100">
          {background}
        </div>
      )}
      {/* Content Container */}
      <div
        className="relative z-10 flex h-full flex-col justify-between p-4 sm:p-5 xl:p-6">
        ...
        {(title || description || icon) && (
          <div
            className={cn("flex items-start space-x-4", children ? "mt-4 sm:mt-6" : "")}>
            {icon && (
              <div
                className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-neutral-200/50 dark:bg-neutral-800/50 text-neutral-900 dark:text-neutral-100">
                {icon}
              </div>
            )}
            <div>
              {title && (
                <h3
                  className="text-lg font-medium tracking-tight text-neutral-900 dark:text-neutral-100">
                  {title}
                </h3>
              )}
              {description && (
